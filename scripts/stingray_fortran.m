function [srRays] = stingray_fortran(srModel, srInitialize, srArc, srControl, srRays, aStation, srStation)
% EDITED BY MALEEN KIDIWELA TO CREATE BLOCKING REGIONS
...
% Modified by M. Kidiwela to add the dPb operation for Orca.

%% Check number of input arguments conical newBlock
...
% Modified by M. Kidiwela to add the dPb operation for Orca.
% comment out if running anisotropy or isotropy for Pg waves

if ismember(srRays.phase,{'dPb'})
%    zh=1;       
    AddBlocking_wall
    
    Sindex=find(ismember(aStation,srStation.name));

    d=find(srModel.zg>-0.6);

    for i=1:length(d)
        jk=d(i);
        extract= srModel.P.u(:,:,jk);
        extract(Blockwc(18).stat(Sindex).tf')= 100000;
        srModel.P.u(:,:,jk) = extract;
    end

    dep=find(srModel.zg>=-4 & srModel.zg<=-1);
     
    for i=1:length(dep)
        jk=dep(i);
        extract= srModel.P.u(:,:,jk);
        extract(Blockwc(i).stat(Sindex).tf')= 100000;
        srModel.P.u(:,:,jk) = extract;
    end

    dep2 =find(srModel.zg<-4);
    for i=1:length(dep2)
        jk=dep2(i);
        extract= srModel.P.u(:,:,jk);
        extract(Blockwc(length(dep)).stat(Sindex).tf')= 100000;
        srModel.P.u(:,:,jk) = extract;
    end
    

elseif ismember(srRays.phase(1),{'P'})    
    
    AddBlocking_cyllinder
    
    deps=find(srModel.zg<=-0.6); %depth of the blocking model
    
    for i=1:length(deps)
        
        if abs(srModel.zg(deps(i)))>=0.6 %depth of the blocking model
            
            jk=deps(i);
            extract= srModel.P.u(:,:,jk);
            
            if i<=length(meshs)
                extract(meshs(i).tf)= 100000;
            else
                ii=length(meshs);
                extract(meshs(ii).tf)= 100000;
            end
            srModel.P.u(:,:,jk) = extract;
        end
    end
...
tic
!/Users/earthnote/Documents/GitHub/StingrayGIT/source/stingray_src_unix
toc
